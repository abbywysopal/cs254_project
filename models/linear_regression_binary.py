...
class MLP(Module):
    ...
    def __init__(self, n_inputs):
        ...
        self.act1 = ReLU()
        # second hidden layer
        self.hidden2 = Linear(20, 16)
        kaiming_uniform_(self.hidden2.weight, nonlinearity='relu')
        self.act2 = ReLU()
        # third hidden layer and output
        self.hidden3 = Linear(16, 14)
...
def train_model(train_dl, model):
    ...
    for epoch in range(len(training_set)):
        ...
        inp = torch.from_numpy(np.asarray(inputs))
        ...
        # yhat = torch.from_numpy(np.asarray(yhat))
...
def evaluate_model(test_dl, model):
    ...
    for epoch in range(len(test_set)):
        ...
        for i in range(len(yhat)):
            dec_actual += str(int(actual[i]))
            dec_yhat += str(int(yhat[i]))
