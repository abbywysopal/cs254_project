...
## formatting for http://x.dii.unisi.it:8098/~giorgi/WebRISC-V/index.php
...
def generate_i(name):
    ...
    if name in SHIFT_IMMEDIATE_INSTRUCTION_NAMES:
        ...
        instruction_assembly = name + " " + str(rd_decimal) + ", " + str(imm_decimal) + "(" \
            + str(rs1_decimal) + ")"
    else:
...
def generate_s(name):
    ...
    instruction_assembly = name + " " + str(rs2_decimal) + ", " + str(imm_decimal) + "(" \
        + str(rs1_decimal) + ")"
    add_instructions(instruction_binary, instruction_assembly, convert_to_hex(instruction_binary))
...
def generate_sb(name):
    ...
    instruction_assembly = name + " " + str(rs1_decimal) + ", " + str(rs2_decimal) + ", 0x" + str(
        imm_decimal)
...
for test_case in range(int(TEST_CASES_NUMBER)):
    ...
    binary_file = open("data/binary" + str(test_case + 1) + ".txt", "w")
    assembly_file = open("data/assembly" + str(test_case + 1) + ".s", "w")
    hex_file = open("data/hex" + str(test_case + 1) + ".v", "w")
    ...
    for i in range(Instructions_Number):
        ...
        if i % 4 == 0 and i != 0:
            hex_file.write("\n")
        hex_file.write("0x" + split_hex[3] + ' ' + split_hex[2] + ' ' + split_hex[1] + ' ' + split_hex[0] + ' ')
